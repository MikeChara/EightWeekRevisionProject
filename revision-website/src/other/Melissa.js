...
const Melissa = () => {
  ...
  const [wasTaxedAdditionalRate, setWasTaxedAdditionalRate] = useState(0);
  const [wasTaxedUpperRate, setWasTaxedUpperRate] = useState(0);
  const [wasTaxedOrdinaryRate, setWasTaxedOrdinaryRate] = useState(0);

  function computeDividendTaxAllowance(
    dividendIncome,
    dividendAllowance,
    wasTaxedAdditionalRate,
    wasTaxedUpperRate,
    wasTaxedOrdinaryRate
  ) {
    setWasTaxedAdditionalRate(0);
    setWasTaxedUpperRate(0);
    setWasTaxedOrdinaryRate(0);
    const result = [];
    ...
    if (remainingDividendIncome <= 150000) {
      ...
      result.push(
        `You were taxed £${wasTaxedUpperRate} at the additional rate.`
      );
      result.push(
